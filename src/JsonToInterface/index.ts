...
import { deleteExt } from '../autoFillContent/utils';
...
export default function jsonToTsInterface(uri: Uri) {
  ...
  const pathNoExt = deleteExt(fsPath,'.json');
  const filePath = `${pathNoExt}Interface.ts`;
  const allInterface = getAllInterface(names, typeStructure);
  fs.writeFileSync(filePath, interfaces.join('\n\n'));
  const modalPath = `${pathNoExt}Modal.ts`;

  let modals = paseInterface(names, typeStructure, 'modal').join('\n\n');
  modals = `import {${allInterface.join(' , ')}} from './${path.basename(filePath)}';\n\n${modals}`;
